...
(describe "->datomic-db"
  (let [db (->datomic-db dm/in-memory-uri)]
    ...
    (it "Does not throw when created twice in a row"
      (should-not-throw (->datomic-db dm/in-memory-uri))
      (should-not-throw (->datomic-db dm/in-memory-uri)))

    (do (dm/recreate-db)
        (for [n (range 5)]
          (it (format "Persists data for %d items" n)
            (should= n (count (find-all-games (->datomic-db dm/in-memory-uri))))
            (save-game (->datomic-db dm/in-memory-uri) {} {} {}))))))
